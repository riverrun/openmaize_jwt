defmodule OpenmaizeJWT.PlugTest do
  ...
  import OpenmaizeJWT.{Plug, TestConn, Verify}
  ...
  test "add token" do
    ...
    conn = conn(:get, "/") |> sign_conn |> add_token(user, :username)
    assert conn.status == 200
    {:ok, token} = get_token(conn) |> verify_token(conn.secret_key_base)
  ...
  test "token with custom unique_id" do
    ...
    conn = conn(:get, "/") |> sign_conn |> add_token(user, :email)
    assert conn.status == 200
    {:ok, token} = get_token(conn) |> verify_token(conn.secret_key_base)
  ...
  test "token with additional data" do
    ...
    {:ok, %{iss: iss}} = get_token(conn) |> verify_token(conn.secret_key_base)
  ...
  test "token without role" do
    ...
    conn = conn(:get, "/") |> sign_conn |> add_token(user, :username)
    assert conn.status == 200
    {:ok, %{id: id}} = get_token(conn) |> verify_token(conn.secret_key_base)
    assert id == 1
  end
